...
public class AppTest {
  ...
  //Creating object with common request specifications like headers and content-type
	...
	public void getAllVotes() {
		...
		response = given()
				.spec(requestSpec)                          // using common specifications
				.get()
				.then()
				.assertThat()
		...
		Vote[] votes = response.body().as(Vote[].class);    //Extracting first vote randomly and save it class level
		randomVote = votes[0];
  ...
  //Extract the getVote response
		response = given()
				...
				.when()
				.get("{pathParamGetVote}")                            // set up endpoint
				.then()
				.body("$", not(empty()))                              // validate response body is not empty
				.extract().response();

		Assert.assertEquals(response.statusCode(), 200);              // validate getVote api response status code
  ...
  //Sending post vote api and verify status code and extracting response
		...
		newlyPostedVoteId = response.getBody().jsonPath().getInt("id");    //save newly posted vote it into global to access in next request
  ...
  //validate getVote for newly posted vote, status code and postedID
		response = given()
				.spec(requestSpec)                                         // using common specifications
				.pathParam("voteIdToPost", newlyPostedVoteId)                   // setup path
				.when()
				.get("{voteIdToPost}")                                     // sending get request
				.then()
				.statusCode(200)                                           // validate status code
				.body("id", equalTo(newlyPostedVoteId))                    // validate id of get response with previously posted id
				.extract().response();
	...
	public void deleteVote() {
		...
		deletedVoteId = newlyPostedVoteId;                                 // setting up deletedVote variable before deleting vote

		given().spec(requestSpec)                                          // using common specifications
				.pathParam("voteIdToDelete", newlyPostedVoteId)            // setup path to previously posted vote
				.when()
				.delete("{voteIdToDelete}")                                // setup endpoint
				.then()
				.statusCode(200)                                           // validate delete api and status code and message
				.body("message", equalTo("SUCCESS"));
  ...
  //Validate get api after deleting vote
		given()
			.spec(requestSpec)
			.pathParam("deletedVote", deletedVoteId)                     // set up path param
				.when()
				.get("{deletedVote}")                                    // set up endpoint
				.then()
				.statusCode(404)                                         // validate status code for get api for deleted vote
